...
> Последнее обновление: **2026-05-07** (E10.4 motion blur + E11 rotated plasma cone muzzle blast landed. post.frag = 5×5 velocity dilation + weighted 8-tap blur + length-clamp; пули теперь читаются как непрерывные streak'и, asteroids получают вертикальный smear. Overlay velocity-clobber fix (cbAtts[1].colorWriteMask=0 для 8 non-opaque pipelines) сохраняет underlying motion vectors при overdraw. Bullet trail VFX переделан: периодические trail-плёшки удалены, hit flash для прямых попаданий, AoE использует существующий fireball+sparks. E11 — drawPlasmaBillboard(rotation) + procedural cone-fan mesh + 3-cone trefoil muzzle blast at 120° (FBM turbulence работает on rotated cone). Side turrets теперь cannon-style: 1 shot/sec, heavy bullet, AoE 0.5, ×3 damage. Раньше: E10.3 prev-frame matrices + per-object velocity (UBO 4 mat4, dynamic-offset UBO set 2 для prev_model). E10.2 velocity attachment infrastructure. E10.1 offscreen render target.)
