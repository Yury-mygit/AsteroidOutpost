...
> Последнее обновление: **2026-05-07** (E10.2 velocity attachment infrastructure landed — второй color attachment `R16G16_SFLOAT` во scene pass, все 9 scene-пайплайнов получили 2-й blend attachment, фрагмент-шейдеры пишут placeholder zero velocity, post descriptor set расширен до 2 bindings. Visually идентично — реальный compute velocity ждёт E10.3 prev-matrices. Раньше: E9 native particle system landed — 2 pipelines additive+alpha-textured, instanced drawParticles API, 3 consumers: AoE sparks, asteroid death debris+smoke, muzzle micro-sparks. E10.1 offscreen render target + post-process passthrough — render flow restructured to scene→offscreen→post→swapchain.)
...
| E10.3-E10.6 | Motion blur: prev-frame matrices, blur shader, particle prev-pos, verify | engine wave | 🟡 **Запланировано** — UBO + push-const расширены `prev*` matrices, vertex shaders пишут реальный screen-space velocity = (currClip - prevClip)/w; particle layout 8→14 floats с prevPos; 8-tap motion-blur shader с length-clamp в NDC. Решит motion aliasing — пули перестают мерцать на 60Hz. |
