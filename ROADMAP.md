...
> Последнее обновление: **2026-05-07** (E10.3 prev-frame matrices + per-object velocity landed — UBO расширен 4 mat4, per-draw dynamic UBO set 2 держит prev_model per draw call. triangle.vert считает реальную screen-space NDC velocity, triangle.frag пишет её в attachment (frame/plasma branches force zero). 5 mesh draw API получили optional prevModelMatrix. Asteroid/Bullet/Fireball трекают prev в Kotlin. Visually идентично — motion blur shader (E10.4) ещё не подключён, но velocity attachment теперь реальный. Раньше: E10.2 velocity attachment infrastructure (R16G16_SFLOAT 2-й color attachment), E10.1 offscreen render target + post-process passthrough — render flow restructured to scene→offscreen→post→swapchain. E9 native particle system.)
...
| E10.4-E10.6 | Motion blur: blur shader, particle prev-pos, verify | engine wave | 🟡 **Запланировано** — 8-tap motion-blur post shader семплит scene colour вдоль vVelocity; particle layout 8→14 floats с prevPos; length-clamp в NDC. Решит motion aliasing — пули перестают мерцать на 60Hz. |
