...
> Последнее обновление: **2026-05-17** (`style3`). Сессия «бой + кампания + рельсотрон». 1) HP-бары с щитом теперь правильного цвета: новый `quadHpShieldHandle` (per-vertex cyan), отдельный handle используется только для верхней полоски щита у `Asteroid.shieldHp > 0` (раньше обе бары делил один зелёный меш, попытка `tintRGBA` на opaque pipeline молча игнорировалась). 2) Кампания (миссии 1-5) переведена в route-mode. Новый процедурный генератор `MissionRoutes.generateUniformCorridor` принимает произвольные `typeWeights`, объекты `CAMPAIGN_1..CAMPAIGN_5` нарастающей длины и плотности — каждый коридор онбордит новый тип астероидов (NORMAL → +FAST → +HEAVY/ENERGY → +EXPLOSIVE → all five). Миссия 6 «первый коридор» удалена (дубликат). Тексты миссий переписаны под полёт по коридору. `RandomMissionsOverlay` теперь показывает только combat-эвенты M7/M8. На экране Victory кнопка «Следующая миссия» появляется только внутри кампании (M1→M2..M4→M5). 3) Muzzle fix центральной турели: единая константа `content.CENTRAL_CANNON_Z_ABOVE_PLATFORM = TURRET_BASE_Y_HEIGHT + TURRET_TOWER_Y_HEIGHT + TURRET_CANNON_HALF_THICK + 0.015` — раньше `SceneAssembler` собирал Z cannon из 4 слагаемых in-place, а `MissionRunner` спавнил muzzle на `CENTRAL_TURRET_BASE_Z` (Z пивота, ~0.32 ед ниже cannon). Дистанция от пивота тоже исправлена: `TURRET_CANNON_LENGTH = 0.55` вместо `CENTRAL_TURRET_HALF_H * 2 = 0.60`. Flash теперь точно у кончика ствола. 4) Weapon catalog переработан. `HEAVY_CANNON` ныне «Пушка» (displayName/description; enum id оставлен историческим) — фугасный AoE-снаряд, тёплый cone-trefoil muzzle (`spawnMuzzleBlast`); lightning-эффект больше не его. Новый `WeaponId.RAILGUN` — «Рельсотрон»: `fireInterval=2.0`, `damageMul=5.0`, `projectileSpeed=108` (≈ 6× автомата), `aoeRadius=0`, slim `Bullet.glb` с per-vertex cyan-blue, lightning muzzle (`spawnRailgunMuzzle`/`Sparks`). Синий трассер от точки выстрела до текущей позиции снаряда — через существующий beam pipeline (без engine-изменений). 5) Tracer ship-tracking: `Projectile` получил `originX/Y/Z`, `spawnShipPosY`, `trailColor`, `trailWidth`. `SceneAssembler` сдвигает `startY` на `shipPosY - spawnShipPosY` чтобы хвост ехал с кораблём, не отставая в хвост корпуса. `RAILGUN_TRAIL_FORWARD_GAP = 0.9` — origin поодаль от дула, чтобы хвост не задевал турель/обшивку. 6) Swept (segment-vs-AABB) collision в `Projectile.tick` — был point-sample, при скорости 108 ед/с (1.7 ед за кадр > радиуса астероида ≈ 0.5) пробивал астероиды насквозь без `onImpact`. Стандартный slab test, snарad снэпается в точку входа перед `onImpact`. Заодно убрал риск туннелирования для будущих fast-projectile оружий. Engine не трогался; все правки — Kotlin. (предыдущее: 2026-05-10 — `style3` ветка — радикальный 3D-pivot. Камера развёрнута за корму корабля (orbit `+0.6` вместо `-0.6`); астероиды летят прямо в нос с дальнего +Z, корабль стоит в нижней половине экрана наклоненной палубой к игроку. **E17** emissive twinkling stars: pipeline `m_starPipeline` рисует instanced point-list через push-constant sentinel `pc.textureMode = 2.0`, fragment-shader делает синусную пульсацию яркости + mask по `vNdcY` (звёзды гаснут в технической полосе под action bar). 600→1500 stars, brightness 0.7-1.0. **E18** fullscreen FBM-фон (`background.vert/.frag`, отдельный pipeline) — реализован, но **отключён** в `MainActivity` из-за просадки FPS 50→19; код оставлен на случай pre-baked-версии. **E19** способность «Дроны» (`AbilityId.DRONES`, cost 40, cd 20с): 4 перехватчика стартуют из-под корабля, держат орбитальную дистанцию (`DRONE_KEEP_DISTANCE`) с per-drone угловым смещением по `swarmIndex` — никогда не слипаются; стреляют зелёным непрерывным лазером через `Beam` с замыканиями source/aim на дрона; при смерти цели переключаются на ближайшую; жизнь 10с. Меш — `art/ship.gltf`, tint красно-оранжевый, yaw offset `-π/2`. В `MissionRunner.tick` дроны обновляются ДО эффектов, чтобы `Beam.aimSelector` видел свежий `targetId`. **Mission flow** переработан: меню → mission hub (Кампания / Случайные миссии вкладки) → graph-style кампания (5 пронумерованных круглых кнопок 78dp с зигзаг-раскладкой и цветом по сложности) → mission detail (карточка с заголовком/статами/описанием/Start) → game. WeaponSelect перед миссией убран — оружие выбирается на «Корабль» (бывшая «База»), сохраняется в `GameProgress.selectedWeaponId` через `SharedPreferences("outpost_progress_v2", "selected_weapon")` и читается при старте миссии. Кнопка «назад» — плавающий 44dp chevron в правом нижнем углу каждого overlay (`attachFloatingBackButton` в `OverlayCommon`). Цветовая палитра: астероиды коричневые (0.78,0.50,0.22 / 0.85,0.55,0.25), дроны красно-оранжевые (0.95,0.40,0.18); туманности перенесены за плоскость спавна астероидов, чтобы их translucency не «серила» сквозные модели. FPS-toggle FAST/FULL по тапу на лейбл FPS (через `engineView.setOnTouchListener` с проверкой координат). VFX: добавлен `spawnRocketLaunchPuff` — облачко дыма из-под корабля при запуске дронов. Camera fov 28°→55°, radius 22→11, target.z 4→2.5. M10–M15 + E13/E14 предыдущая волна — см. ниже.) (предыдущее: 2026-05-09 — M10 UI/щит-полировка: иконки на ability-buttons (V-щит, ракета, лазер-режет-астероид), щит-кнопка с green/gray HP-fill вертикальной полосой, action-bar 32dp icon-only, форма щита-арки = superellipse n=4 (плоский верх, резкие плечи) с подъёмом 5%, новая иконка приложения (заменяет g3-зелёную). M11 процедурные платформенные меши: турели расщеплены на статичную базу + вращающуюся башню (`buildTurretBaseMesh` + `buildTurretBarrelMesh`), лазерный купол (ground-telescope-style) и ракетная шахта (open-hatch with launch tube) рядом с центральной турелью. M12 непрерывный лазер (5 сек, 50 dps, blocks-on-first-asteroid) заменяет старый drag-line strike; полностью выпилен legacy strike + armed-target framework. **E13** plasma billboard матрица rotation+non-uniform-scale fix — добавлен `Mat4::scale`, переписана композиция в `VulkanContext::renderFrame` plasma-секции на `billboard_uniform × Ry(rot) × S(scaleH,1,scaleV)`. **E14** dedicated `drawLaserBeam` API — новый Vulkan pipeline `m_beamPipeline` с собственным layout, `beam.vert/.frag` GLSL шейдеры, GPU-side view-aligned quad expansion, sharp endpoints, gentle pulse. Через C/JNI/Kotlin → `BeamDraw` data class + `EngineView.beams`. M13 WeaponEffect umbrella — refactor `bullets: List<Bullet>` → `effects: List<WeaponEffect>`, единый `tick(dt) → consumed`-loop. Конкретные классы: `Projectile` (с `behaviour: ProjectileBehavior` strategy), `Beam` (с closure source/aim для g3-портабельности). M14 ракетный overhaul: процедурный меш (`buildRocketMesh`: engine bell + body + 2 fins + nose + warning stripe), spring-launch sequence (queue в RocketSilo, ASCENDING фаза straight-up, ASCENT_HEIGHT=2×length, FLYING фаза = boost+steer), 3-фазный VFX (no-engine во время spring → яркий ignition flash на переходе → реактивный jet + smoke trail в FLYING). M15 per-weapon firing arcs (90/80/80/70/95/95) + priority-lock semantics (master target для центра+лазера, re-tap toggles release, центр клампит вращение в свою дугу но всё равно ведёт цель, лазер фолловит того же таргета через canEngage closure в Beam). Также: астероиды разбиваются о щит-арку через superellipse Z(x), не долетая до базы; -20% урона по щиту во время recharge + cyan tangential искры по дуге. M9 ↑ выше: M8 + M9 геймплейный pivot. Центральная турель больше не управляется вручную — auto-aim с sticky lock на самую "толстую" цель + tap-to-priority. HP-bars над повреждёнными астероидами. Энергия как ресурс под способности (100 max, +10/sec). Две способности: Ракетный залп (3 homing missiles, 30 энергии, cd 8с) + Лазерный удар (drag-line, 5 lightning bolts, 50 энергии, cd 18с). Ability framework — `game/Ability.kt` + `AbilityCatalog`, runtime в `AbilitySlot`. UI: ability bar рядом с щитом. Threading — все mutate-list ability activations marshalled на mission thread через `missionHandler.post`. Mission counts ×1.5–2 + интервалы спавна тактнее. M9 — щит переработан: убран ShieldState (READY/ACTIVE/COOLING), теперь permanent HP-based barrier (max 500). Урон астероидов идёт сначала в щит, overflow на платформу. Кнопка ЩИТ — hold-to-recharge: 50 энергии/сек → 200 HP/сек (4× ratio). Гео — `buildShieldArchMesh` строит wide flat ellipse arch с вершинами в мировых координатах (constant-thickness band, никакой anisotropy от scaleX≠scaleZ). Раньше: E12 railgun muzzle lightning + concept rename Тяжёлая пушка→Рельсотрон. E11 — drawPlasmaBillboard(rotation) + cone-trefoil muzzle blast + side turrets cannon-style. E10.4 motion blur post.frag.)
