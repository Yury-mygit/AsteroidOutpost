...
Зависимости: M1 → {M2, M3, M5}; M2 → M4; {M2, M3, M5} → M6; всё → M7. E1 параллельно (затрагивает только нативку).
...
## Бэклог по движку (E2-волна — после E1)

E1 закрыл базовую прозрачность и процедурные меши (см. выше). Что осталось — для следующей волны движка, когда снова упрёмся в визуальный потолок:

- **UV-координаты + текстуры.** Сейчас вершина = `pos + rgba + normal`. Нет UV → нельзя сэмплить текстуру в фрагменте → не получится нормальный «облачный» нéбул-шейдер, тинтованные иконки, спрайтовые VFX. **Нужно:** второй vertex-attribute UV (`vec2 @ location 3`) во всех пайплайнах, descriptor-set с `combined image sampler`, API `load_texture(bytes)` + `draw_textured_mesh(token, textureToken, mat4)`. Большая правка — затронет `Mesh`, `VulkanContext`, шейдеры, C API.
...
- **Софт-фейд по краям plasma-биллборда.** Plasma-квад всё ещё однородно-яркий (E1 не трогал plasma-shader). Эффект: ступенчатый силуэт у щит-купола. **Нужно:** в plasma-fragment-шейдере добавить радиальный fade на основе UV (`alpha = smoothstep(1.0, 0.6, length(uv*2-1))`). Зависит от UV-задачи выше.
