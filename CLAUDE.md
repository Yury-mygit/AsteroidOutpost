...
- **Player input is priority + abilities**, not aiming. Central turret auto-targets highest-current-HP asteroid (sticky lock until kill). Tap an asteroid → priority-lock; re-tap to release. Locked target is master for both central turret AND active laser beam.
